...
from flaggems_vllm.ops.topk_softplus_sqrt import HAS_TLE, topk_softplus_sqrt
...
# Gems non-TLE entry point (original kernel, `use_ascend_tle=False`) --
# pinned via functools.partial for the same reason as `_gems_tle_op` below,
# so both tests share one calling convention.
...
class TopkSoftplusSqrtBenchmark(base.Benchmark):
    """Gems baseline (non-TLE) kernel vs. the Torch/vLLM reference."""
...
class TopkSoftplusSqrtTleBenchmark(TopkSoftplusSqrtBenchmark):
    """Same shapes/inputs as the baseline benchmark; only the op differs."""


@pytest.mark.topk_softplus_sqrt
def test_topk_softplus_sqrt():
    """Original baseline: gems (use_ascend_tle=False) vs. Torch/vLLM reference."""
...
def test_topk_softplus_sqrt_tle():
    """TLE-optimized: gems (use_ascend_tle=True) vs. Torch/vLLM reference."""

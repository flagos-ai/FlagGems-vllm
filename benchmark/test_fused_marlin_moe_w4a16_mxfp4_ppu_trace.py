...
"""MXFP4 benchmark on the PR5140 production trace.

E=256, K=4096, N=256, topk=6, BF16 and all 53 token counts/call counts
follow FlagGems PR #5140. Baseline is native vLLM Marlin MXFP4. Use
--mode kernel --warmup 1000 --iter 100, or --mode cudagraph --iter 100.
"""
...
def _to_vllm_marlin(weight, scales, size_k, size_n):
    ...
    # The input already contains E8M0 codes. Match the vLLM MXFP4
    # [0,2,1,3] scale permutation without a numeric float8 conversion.
    scales = (
        scales.reshape(-1, 4)[:, [0, 2, 1, 3]]
        .reshape(weight.size(0), size_k // GROUP_SIZE, size_n)
        .contiguous()
...
class FusedMarlinMoEW4A16MXFP4Benchmark(base.Benchmark):
    ...
    def get_input_iter(self, dtype):
        ...
        for num_tokens, call_count in PR5140_TRACE:
            ...
            # Validate the same raw weights and routes before timing each
            # production-trace shape. Packing and validation are not timed.
...
def _gems_call(
    ...
    return flaggems_vllm.fused_marlin_moe_w4a16_mxfp4(
        hidden_states,
        w1,
        w2,
        None,
...
@pytest.mark.skipif(flaggems_vllm.vendor_name != "thead", reason="PPU trace benchmark")
@pytest.mark.skipif(

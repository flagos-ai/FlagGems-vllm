...
"""FP8 benchmark on the PR5140 production trace.

E=256, K=4096, N=256, topk=6, BF16 and all 53 token counts/call counts
follow FlagGems PR #5140. Baseline is native vLLM Marlin FP8. Use
...
def _to_vllm_marlin(weight, scales, size_k, size_n):
    ...
    qweight = vllm_ops.gptq_marlin_moe_repack(
        qweight, empty_perm, size_k=size_k, size_n=size_n, num_bits=8
    )
...
def _make_weights(num_experts, hidden_size, intermediate_size, dtype):
    ...
    for no, ki in (
        ...
        scale = (
            torch.rand((num_experts, no, ki // GROUP_SIZE), device=flaggems_vllm.device)
            * 0.001
            + 0.0005
        ).to(dtype)
        nw, ns = _to_vllm_marlin(weight, scale, ki, no)
        weights.append(weight)
        scales.append(scale)
        native_weights.append(nw)
        native_scales.append(ns)

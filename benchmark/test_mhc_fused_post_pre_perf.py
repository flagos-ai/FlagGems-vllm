...
def _optimized_device_available() -> bool:
...
def _capture(
...
) -> tuple[torch.cuda.CUDAGraph, list[tuple[torch.Tensor, ...]]]:
    ...
    graph.replay()
    torch.cuda.synchronize()
    # Keep the captured outputs alive for as long as the graph is timed.  CUDA
    # graph allocations come from a private pool, but the Tensor objects still
    # own the captured addresses; dropping them here can make a later capture
    # reuse storage referenced by this graph.
    return graph, holder
...
def _run_case(
    ...
    baseline_graph, baseline_outputs = _capture(
        lambda: _baseline(inputs), copies, warmup
    )
    candidate_graph, candidate_outputs = _capture(
        lambda: _candidate(inputs), copies, warmup
    )
    # The names deliberately remain live through the complete ABBA timing.
    assert baseline_outputs and candidate_outputs
    return {
...
def test_mhc_fused_post_pre_performance() -> None:
    ...
    if not _optimized_device_available():
        pytest.skip("the optimized mHC benchmark requires SM90 or newer")
...
def main() -> None:
    ...
    if not _optimized_device_available():
        raise RuntimeError("this benchmark requires SM90 or newer")
    _register_vllm_baseline()

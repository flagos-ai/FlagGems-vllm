...
from flaggems_vllm.ops.mhc.mhc_post_optimized import _mhc_post_impl
...
def mhc_fused_post_pre(
    ...
    norm_eps: float = 1e-6,
    *,
    enable_fused_post_pre_optimization: bool = False,
    ...
    specialization internally. Before CUDA Graph capture, call
    ``mhc_prepare_weights(fn, enable_fused_post_pre_optimization=True)`` once
    for each immutable ``fn`` tensor so its Triton-packed cache is ready.
    """
    if enable_fused_post_pre_optimization is not True:
        raise NotImplementedError(
            "the optimized mHC post-to-pre path is disabled; pass "
            "enable_fused_post_pre_optimization=True explicitly"
        )

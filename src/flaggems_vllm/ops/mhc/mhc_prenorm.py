...
def mhc_prepare_weights(
    ...
    *,
    ...
    if enable_fused_post_pre_optimization is not True:
        raise NotImplementedError(
            "optimized mHC weight preparation is disabled; pass "
...
def mhc_prenorm_gemm(
    ...
    *,
    ...
    capture requires an enabled warm-up call or
    ``mhc_prepare_weights(..., enable_fused_post_pre_optimization=True)`` for
    each immutable ``fn`` identity. No PyTorch compute or copy kernel is used
    by this production path.
    """
    if enable_fused_post_pre_optimization is not True:
        raise NotImplementedError(
            "optimized mHC prenorm is disabled; pass "
            "enable_fused_post_pre_optimization=True explicitly"
        )

...
def fused_marlin_moe_w8a16_fp8(
    ...
    *,
    ...
    """Run fused Marlin MoE with FP8 E4M3 weights and A16 activations.
    ...
    ``w1=(E, 2I, H)`` and ``w2=(E, H, I)`` with groupwise scales.
    """
    assert hidden_states.dtype in (torch.float16, torch.bfloat16)
    assert hidden_states.is_contiguous()
    ...
    if num_tokens <= 16:
        bsm_block_m = 2 if num_tokens == 1 else 4
...
@triton.jit
def fused_moe_kernel_w8a16_int8_gateup(
    A,  # (T, H) bf16, indexed by token_id
    W1_q,  # (E, Nw1, H) uint8
    W1_scales,  # (E, Nw1, H_groups) bf16
    W1_zp,  # (E, Nw1, H_groups) uint8 or empty
...
@triton.jit
def fused_moe_kernel_w8a16_int8_down(
    INTER,  # (M_padded, I) bf16, indexed by dispatch_idx
    W2_q,  # (E, H, I) uint8
    W2_scales,  # (E, H, I_groups) bf16
    W2_zp,  # (E, H, I_groups) uint8 or empty
...
@triton.jit
def fused_moe_kernel_w8a16_int8_unified_moe(
    A,
    W1_q,
    W1_scales,
    W1_zp,
...
@triton.jit
def fused_moe_kernel_w8a16_int8_unified_moe_per_m(
    A,
    W1_q,
    W1_scales,
    W1_zp,
...
def _launch_w8a16_gateup_silu_int8(
    x,
    W1_q,
    W1_scales,
    zp1,
    ...
    *,
    ...
    if pin is not None:
        ...
        _mxq_triton_jit_fn(fused_moe_kernel_w8a16_int8_gateup_silu)[_grid_pin](
            x,
            W1_q,
            W1_scales,
            zp1,
    ...
    fused_moe_kernel_w8a16_int8_gateup_silu[_grid_gateup_silu](
        x,
        W1_q,
        W1_scales,
        zp1,
...
def _launch_w8a16_down_int8(
    ...
    *,
    ...
    if pin is not None:
        ...
        _mxq_triton_jit_fn(fused_moe_kernel_w8a16_int8_down)[_grid_down_pin](
            intermediate,
            W2_q,
            W2_scales,
            zp2,
    ...
    fused_moe_kernel_w8a16_int8_down[_grid_down](
        intermediate,
        W2_q,
        W2_scales,
        zp2,
...
def invoke_fused_moe_full_swiglu_int8(
    x: torch.Tensor,
    W1_q: torch.Tensor,
    W1_scales: torch.Tensor,
    W1_zeros: Optional[torch.Tensor],
    ...
    # autotune list divides the contraction dim.  Computed per-kernel because
    # the fused kernel uses a different config list.
    _bsks_legacy = {c.kwargs["BLOCK_SIZE_K"] for c in _W8A16_AUTOTUNE_CONFIGS_INT8}
    _bsks_fused = {c.kwargs["BLOCK_SIZE_K"] for c in _W8A16_FUSED_AUTOTUNE_CONFIGS_INT8}
    _bsks_fused_large = {
        c.kwargs["BLOCK_SIZE_K"] for c in _W8A16_FUSED_LARGE_AUTOTUNE_CONFIGS
    }
    _bsks_down = {c.kwargs["BLOCK_SIZE_K"] for c in _W8A16_DOWN_AUTOTUNE_CONFIGS_INT8}
    if use_fused_gateup_silu:
        even_Ks_gateup = all((H % bsk) == 0 for bsk in _bsks_fused)
    else:
        even_Ks_gateup = all((H % bsk) == 0 for bsk in _bsks_legacy)
    ...
    if use_unified_moe_path:
        ...
        if _mxq_use_unified_mi_fusion(num_valid_tokens):
            ...
            fused_moe_kernel_w8a16_int8_unified_moe[_grid_unified_mi](
                x,
                W1_q,
                W1_scales,
                zp1,
            ...
            if unified_large_mode == "per_m":
                ...
                fused_moe_kernel_w8a16_int8_unified_moe_per_m[_grid_unified_per_m](
                    x,
                    W1_q,
                    W1_scales,
                    zp1,
                ...
                if use_large_gateup_silu:
                    ...
                    _launch_w8a16_gateup_silu_int8(
                        x,
                        W1_q,
                        W1_scales,
                        zp1,
    ...
    if use_fused_gateup_silu:
        ...
        fused_moe_kernel_w8a16_int8_gateup[_grid_gateup](
            x,
            W1_q,
            W1_scales,
            zp1,
...
def fused_marlin_moe_w8a16_int8(
    ...
    *,
    ...
    invoke_fused_moe_full_swiglu_int8(
        hidden_states,
        w1,
        w1_scale,
        w1_zeros,

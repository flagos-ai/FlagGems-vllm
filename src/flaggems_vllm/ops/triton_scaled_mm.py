...
def scaled_mm_kernel(
    ...
    a_ptrs = a_ptr + stride_am * offs_m[:, None] + stride_ak * (offs_k[None, :] + last_k_offset)
    b_ptrs = b_ptr + stride_bk * (offs_k[:, None] + last_k_offset) + stride_bn * offs_n[None, :]
    ...
    scale_a_ptrs = scale_a_ptr + offs_sm + (BLOCK_SIZE_SCALE_A > 1) * pid_m * BLOCK_SIZE_M
    scale_b_ptrs = scale_b_ptr + offs_sn + (BLOCK_SIZE_SCALE_B > 1) * pid_n * BLOCK_SIZE_N

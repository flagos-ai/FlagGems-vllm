...
def _fused_q_kv_rmsnorm_tile_kernel(
    ...
    x = tl.load(
        in_ptr + offs_m[:, None] * in_stride + offs_n[None, :],
        mask=mask,
        other=0.0,
    ).to(tl.float32)
...
def _fused_q_kv_rmsnorm_medium(
    qr: torch.Tensor,
    kv: torch.Tensor,
    q_weight: torch.Tensor,
    kv_weight: torch.Tensor,
...
def _is_musa_medium_n(size: int) -> bool:
    return (
        ...
        and _MUSA_MEDIUM_N_MIN < size <= _MUSA_MEDIUM_N_MAX
    )
...
def fused_q_kv_rmsnorm(
    ...
    if has_large:
        ...
        return _fused_q_kv_rmsnorm_medium(qr, kv, q_weight, kv_weight, eps)
    else:
        return _fused_q_kv_rmsnorm_tile(qr, kv, q_weight, kv_weight, eps)

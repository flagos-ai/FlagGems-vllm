...
"""MetaX C-series-optimized Qwen4 compress+norm+mrope+store fused kernel.
...
def _compress_norm_mrope_store_qsa_groups_kernel(
    ...
    if valid_row:
        ...
        positions = end_position - (COMPRESS_RATIO - 1 - group_offsets)
        logical_pages = positions // RAW_PAGE_SIZE
        page_offsets = positions % RAW_PAGE_SIZE
        valid = logical_pages < RAW_TABLE_WIDTH
        physical_pages = tl.load(
            raw_table_ptr
            + request * stride_table_req
            + tl.minimum(logical_pages, RAW_TABLE_WIDTH - 1) * stride_table_page,
            mask=valid,
            other=-1,
        )
        valid &= (physical_pages >= 0) & (physical_pages < num_raw_blocks)
        values = tl.load(
            raw_cache_ptr
            + tl.maximum(physical_pages, 0).to(tl.int64)[:, None] * stride_raw_block
            + page_offsets[:, None] * stride_raw_token
            + dims[None, :] * stride_raw_dim,
            mask=valid[:, None] & (dims[None, :] < HEAD_DIM),
            other=0.0,
        )
        accumulator = tl.sum(values.to(tl.float32), axis=0)
    ...
    if LOAD_MROPE_POSITIONS:
        ...
        time_position = tl.load(rope_base + 0 * stride_rope_dim, mask=valid_rope, other=0)
        height_position = tl.load(
            rope_base + 1 * stride_rope_dim, mask=valid_rope, other=0
        )
        width_position = tl.load(
            rope_base + 2 * stride_rope_dim, mask=valid_rope, other=0
        )
    else:
...
def qwen4_compress_norm_mrope_store_groups(
    ...
    if rows == 0:
        return out
    load_mrope_positions = rope_cache is not None
    if rope_cache is None:
        rope_cache = raw_cache
    mrope_section = tuple(mrope_section)
    grid = (rows,)
    _compress_norm_mrope_store_qsa_groups_kernel[grid](
        raw_cache,
        rope_cache,
        raw_block_table,
        token_to_req,
        ...
        COMPRESSED_PAGE_SIZE=out.shape[1],

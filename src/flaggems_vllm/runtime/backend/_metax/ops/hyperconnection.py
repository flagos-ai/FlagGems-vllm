...
"""MetaX-optimized Qwen4 HyperConnection inject-combine.
...
@triton.jit
def _hc_inject_combine_fused_kernel(
    injection_logits_ptr,
    block_output_ptr,
    residual_ptr,
    output_ptr,
...
def qwen4_hc_inject_combine(
    ...
    if rows >= 16:
        fused_block_h = 512
        _hc_inject_combine_fused_kernel[
            (rows, triton.cdiv(hidden_size, fused_block_h))
        ](
            *common_args,

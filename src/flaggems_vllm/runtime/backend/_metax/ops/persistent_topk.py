...
SCRATCH_SUFFIX = tl.constexpr(256)
SCRATCH_SCALARS = tl.constexpr(512)
SCRATCH_ORDERED = tl.constexpr(516)
...
def _radix_topk_global(
    ...
    if cta_in_group == 0:
        for buf in tl.static_range(0, 3):
            tl.store(g_histogram_ptr + buf * RADIX + lane, 0, mask=lane < RADIX)
    _barrier_with_atomic_add(
        g_state_ptr + 2,
        zeros,
        lane,
    ...
    for round_idx in tl.static_range(0, 4):
        ...
        for t in tl.range(0, n_vec_full):
            ...
            match = (ordered & mask) == prefix
            bucket = ((ordered >> shift_bits) & 0xFF).to(tl.int32)
            hist_acc += tl.histogram(bucket, RADIX, mask=match)
        for t in tl.range(0, rem_tiles):
            offs = (n_vec_full * VEC_SIZE + t) * BLOCK_SIZE + lane
            ordered = tl.load(shared_ordered_ptr + offs)
            mask = 0 if round_idx == 0 else ((~0) << (32 - round_idx * 8))
        ...
        if rem_elems > 0:
            ...
            match = (ordered & mask) == prefix
            bucket = ((ordered >> shift_bits) & 0xFF).to(tl.int32)
            hist_acc += tl.histogram(bucket, RADIX, mask=match & in_range)
    ...
    cursor_2d = tl.zeros([BLOCK_SIZE, VEC_SIZE], dtype=tl.int32)
    for t in tl.range(0, n_vec_full):
        base = t * BLOCK_SIZE * VEC_SIZE + lane * VEC_SIZE
        offs = base[:, None] + vec[None, :]
        ordered = tl.load(shared_ordered_ptr + offs)
        ...
        tl.store(
            row_output + gt_base_2d + cursor_2d,
            my_chunk_start + offs,
            mask=gt_mask,
        )
        cursor_2d += gt_mask.to(tl.int32)
    cursor_1d = tl.zeros([BLOCK_SIZE], dtype=tl.int32)
    for t in tl.range(0, rem_tiles):
        offs = (n_vec_full * VEC_SIZE + t) * BLOCK_SIZE + lane
        ordered = tl.load(shared_ordered_ptr + offs)
        gt_mask = ordered > ordered_pivot
        tl.store(
            row_output + gt_base_1d + cursor_1d, my_chunk_start + offs, mask=gt_mask
        )
        cursor_1d += gt_mask.to(tl.int32)
    if rem_elems > 0:
        offs = (n_vec_full * VEC_SIZE + rem_tiles) * BLOCK_SIZE + lane
        in_range = lane < rem_elems
        ordered = tl.load(shared_ordered_ptr + offs, mask=in_range, other=0)
    ...
    for t in tl.range(0, n_vec_full):
        ...
        pos = eq_base_2d + cursor_2d
        tl.store(row_output + pos, my_chunk_start + offs, mask=eq_mask & (pos < TOPK))
        cursor_2d += eq_mask.to(tl.int32)
    cursor_1d = tl.zeros([BLOCK_SIZE], dtype=tl.int32)
    for t in tl.range(0, rem_tiles):
        offs = (n_vec_full * VEC_SIZE + t) * BLOCK_SIZE + lane
        ordered = tl.load(shared_ordered_ptr + offs)
        eq_mask = ordered == ordered_pivot
        pos = eq_base_1d + cursor_1d
        tl.store(row_output + pos, my_chunk_start + offs, mask=eq_mask & (pos < TOPK))
        cursor_1d += eq_mask.to(tl.int32)
    if rem_elems > 0:
        offs = (n_vec_full * VEC_SIZE + rem_tiles) * BLOCK_SIZE + lane
        in_range = lane < rem_elems
        ordered = tl.load(shared_ordered_ptr + offs, mask=in_range, other=0)
...
def persistent_topk_kernel_global(
    ...
    #   [0, 256) local_histogram, [256, 512) suffix_sum, [512, 516) scalars,
    #   [516, 516+ordered) shared_ordered (chunk / medium / decode reuse)
    scratch_base = scratch_ptr + pid * SCRATCH_STRIDE
    local_histogram_ptr = scratch_base + SCRATCH_HIST
    suffix_sum_ptr = scratch_base + SCRATCH_SUFFIX
    shared_scalars_ptr = scratch_base + SCRATCH_SCALARS
    shared_ordered_ptr = scratch_base + SCRATCH_ORDERED
...
# ════════════════════════════════════════════════════════════════════════════
# V1 top-k path — per-row single-CTA kernel, dispatched when num_rows > 32 or
...
# Measured on C550 (seq=262144, k=512): per-CTA pass time is about
# T(chunk) ~= 40us + 5us per 1000 elements and the machine runs ~104 barrier
# CTAs concurrently, so the total cost is waves * T(chunk) with
# waves = ceil(num_rows * cpg / 104) and cpg = ceil(stride / chunk).
# Small chunks win for few rows (less work per CTA); larger chunks win once
# multiple waves appear (fewer CTAs per group, fewer barrier arrivals).
_GLOBAL_CHUNK_CANDIDATES = (4096, 8192, 16384, 32768)
_GLOBAL_BARRIER_CAPACITY = 104
_GLOBAL_FIXED_US = 40.0
_GLOBAL_PER_KILO_US = 5.0
# V1 per-row kernel cost (measured): one CTA per row, all rows in parallel,
# ~1.15us per 1000 elements of seq_len (seq=262144 -> ~301us, 131072 -> ~151us).
_V1_US_PER_KILO = 1.15
...
def persistent_topk(
    ...
    # per-row (V1) vs cooperative dispatch, cost-model driven (C550, k=512):
    ...
    #     rows in parallel, ~1.15us per 1000 seq elements).
    # Validated crossovers: seq=262144 -> cooperative wins up to 13 rows
    # (198-257us vs 302-310us), per-row from 14 rows (310us vs 357-392us);
    # seq=131072 -> per-row from ~8 rows (138-160us vs cooperative 198us+).
    per_row_ok = max_seq_len <= RADIX_THRESHOLD or (
        num_rows >= 2
        and _coop_cost_us(num_rows, stride) > _V1_US_PER_KILO * max_seq_len / 1000.0
    )
    if num_rows > 32 or per_row_ok:
        # V1: one CTA per row, buffers in smem (upstream _per_row_cta_* style)

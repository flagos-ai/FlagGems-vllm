...
"""MetaX (MC550) fused MoE: self-contained vendor implementation.

Owns the kernels, dispatch and expert loop instead of patching the generic
op, and inlines the generic non-TLE ``moe_align_block_size`` orchestration
(the generic entry point tries a TLE cooperative kernel whose
``tle.distributed_barrier`` cannot compile on the MetaX backend). Follows
the Hygon/PPU pattern established in PR #761.
"""
...
def _moe_align_block_size(
    ...
    kernel whose ``tle.distributed_barrier`` cannot compile on the MetaX backend
    (failed-JIT cost per call + MLIR error spam), so vendor fused_moe runs
...
def _is_qwen_plain_half_shape(hidden_states, w1, w2, topk_ids) -> bool:
...
def _should_defer_router_weight(
    w1, num_tokens, apply_router_weight_on_input, is_qwen_half
) -> bool:
    """MC550 range selector for the backend-only fused GEMM2+reduction path."""
    if not is_qwen_half or apply_router_weight_on_input:
        return False
...
def _generic_get_default_config(
    M: int,
    E: int,
    N: int,
    K: int,
...
def invoke_fused_moe_triton_kernel(
    ...
    _selected_kernel = (
        _metax_pair_fused_moe_kernel if pair_gate_up_dot else fused_moe_kernel
    )
    _selected_kernel[grid](
        A,
        B,
        C,
        B_bias,
...
def fused_experts_impl(
    ...
    is_qwen_half = _is_qwen_plain_half_shape(hidden_states, w1, w2, topk_ids)
    defer_router_weight = _should_defer_router_weight(
        w1, num_tokens, apply_router_weight_on_input, is_qwen_half
    )
    E, N, _ = w1.size()
    K = w2.size(1)
    ...
    for chunk in range((num_tokens // CHUNK_SIZE) + 1):
        ...
        dispatch_fused_moe_kernel(
            ...
            expert_ids,
            num_tokens_post_padded,
            not apply_router_weight_on_input and not defer_router_weight,
            1,
            gemm2_config,
            compute_type=compute_type,
            use_fp8_w8a8=use_fp8_w8a8,
        ...
        if not use_direct_sum:
            moe_sum(
                ...
                curr_topk_weights if defer_router_weight else None,
            )

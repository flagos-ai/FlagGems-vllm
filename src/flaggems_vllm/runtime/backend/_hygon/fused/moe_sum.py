...
@triton.jit
def _hygon_moe_sum_pair_kernel(
    input_ptr,
    output_ptr,
    router_weights_ptr,
    num_tokens,
...
@triton.jit
def _hygon_moe_sum_general_kernel(
    input_ptr,
    output_ptr,
    router_weights_ptr,
...
def moe_sum(
    ...
    if (
        ...
        _hygon_moe_sum_pair_kernel[grid](
            input,
            output,
            input if router_weights is None else router_weights,
            num_tokens,
        ...
        grid = lambda meta: (num_tokens, triton.cdiv(hidden_size, meta["BLOCK_SIZE"]))
        _hygon_moe_sum_general_kernel[grid](
            input,
            output,
            input if router_weights is None else router_weights,

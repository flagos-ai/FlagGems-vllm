...
"""Hygon DCU-optimized Qwen4 compress+norm+mrope+store fused kernel.
...
"""Qwen4 QSA fused compress-norm-mrope-store kernel (self-developed fast path)."""
...
def _compress_norm_mrope_store_qsa_groups_kernel(
    ...
    if valid_row:
        for group_offset in tl.static_range(0, COMPRESS_RATIO):
    ...
    if LOAD_MROPE_POSITIONS:
        ...
        time_position = tl.load(rope_cache_ptr + rope_base, mask=valid_rope, other=0)
        height_position = tl.load(
            rope_cache_ptr + rope_base + stride_rope_dim, mask=valid_rope, other=0
        )
        width_position = tl.load(
            rope_cache_ptr + rope_base + 2 * stride_rope_dim, mask=valid_rope, other=0
        )
    else:
    ...
    valid_time = (time_position >= 0) & (time_position < num_cos_rows)
    valid_height = (height_position >= 0) & (height_position < num_cos_rows)
    valid_width = (width_position >= 0) & (width_position < num_cos_rows)
    safe_time = tl.minimum(tl.maximum(time_position, 0), num_cos_rows - 1)
    safe_height = tl.minimum(tl.maximum(height_position, 0), num_cos_rows - 1)
    safe_width = tl.minimum(tl.maximum(width_position, 0), num_cos_rows - 1)
    cos_time = tl.load(
        cos_sin_cache_ptr + safe_time * stride_cos_row + frequencies * stride_cos_dim,
        mask=valid_row & valid_time,
        other=0.0,
    )
    cos_height = tl.load(
        cos_sin_cache_ptr + safe_height * stride_cos_row + frequencies * stride_cos_dim,
        mask=valid_row & valid_height,
        other=0.0,
    )
    cos_width = tl.load(
        cos_sin_cache_ptr + safe_width * stride_cos_row + frequencies * stride_cos_dim,
        mask=valid_row & valid_width,
        other=0.0,
    )
    cos = tl.where(use_height, cos_height, tl.where(use_width, cos_width, cos_time))
    sin_time = tl.load(
        cos_sin_cache_ptr
        + safe_time * stride_cos_row
    ...
    sin_width = tl.load(
        cos_sin_cache_ptr
        + safe_width * stride_cos_row
        + (ROTARY_DIM // 2 + frequencies) * stride_cos_dim,
        mask=valid_row & valid_width,
        other=0.0,
    )
    sin = tl.where(use_height, sin_height, tl.where(use_width, sin_width, sin_time))
...
def qwen4_compress_norm_mrope_store_groups(
    ...
    tensors = [
        raw_cache, raw_block_table, token_to_req, logical_positions,
        compressed_slots, compressed_cache, norm_weight, cos_sin_cache,
    ]
    ...
    if (raw_cache.ndim != 4 or raw_cache.shape[2:] != (1, 128)
            or not all(raw_cache.shape)):
        raise ValueError("Qwen4 QSA raw cache must be nonempty [blocks, page, 1, 128]")
    if (compressed_cache.ndim != 4 or compressed_cache.shape[2:] != (1, 128)
    ...
    if cos_sin_cache.dtype != raw_cache.dtype or not cos_sin_cache.shape[0]:
        raise ValueError("Qwen4 QSA cos/sin cache must be nonempty and match cache dtype")
    if (len(mrope_section) != 3 or any(section < 0 for section in mrope_section)
            or rotary_dim != 64 or sum(mrope_section) != rotary_dim // 2):
        raise ValueError("Qwen4 QSA MRoPE requires rotary_dim=64 and sections summing to 32")
    if compress_ratio <= 0 or norm_eps <= 0:
        raise ValueError("Qwen4 QSA compression ratio and norm epsilon must be positive")
    if rope_cache is not None and (rope_cache.ndim != 4
            or rope_cache.shape[:3] != raw_cache.shape[:3]
            or rope_cache.shape[3] != 3 or rope_cache.dtype != torch.int64):
        raise ValueError("Qwen4 QSA packed MRoPE cache must be [blocks, page, 1, 3] int64")

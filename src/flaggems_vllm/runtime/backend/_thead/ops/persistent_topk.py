...
# PPU tier for a single row (2026-09-02): 96KB -> max_chunk_elements 24056
# -> chunk 16384 -> ctas_per_group 16 (measured best for num_rows==1:
# cpg16 1.61x vs generic cpg32 1.35x on seq=262144). Full 256KB would give
# cpg=8 (1.36x) for a lone row; 96KB lands on the cpg=16 sweet spot.
SMEM_TIER_R1 = 98304
...
def _barrier_with_atomic_add(
    ...
    # PPU: tl.atomic_add(ptr, 0, sem="acquire", scope="gpu") used as a spin
    # read never observes other CTAs' release writes at >= 8 CTAs (deadlock).
    # A volatile load sees the release-atomically-published counter (verified
    # on PPU-ZW810E: cross-CTA data written before the barrier is visible
    # after the spin).
    arrival_counter = tl.load(arrival_counter_ptr, volatile=True)
    while arrival_counter < thresold:
        arrival_counter = tl.load(arrival_counter_ptr, volatile=True)


@triton.jit
def _radix_prefix_mask(round_idx: tl.constexpr):
    # PPU: the generic `(~0) << shift` form is an int32 value mixed into
    # uint32 key arithmetic; mixed-type AND misbehaves on PPU-ZW810E
    # (rounds >= 1 match almost nothing). Build the mask as uint32.
    if round_idx == 0:
        return tl.full((), 0, tl.uint32)
    return tl.full((), 0xFFFFFFFF, tl.uint32) << (32 - round_idx * 8)
...
def _radix_topk(
    ...
    if cta_in_group == 0:
        tl.atomic_xchg(
            g_state_ptr + 3 + zeros, 0, mask=lane == 0, sem="relaxed", scope="gpu"
        )  # output_counter
    ...
    for round_idx in tl.static_range(0, 4):
        ...
        # PPU: dedicated buffer per round (no 3-buffer rotation). The rotation's
        # CTA0 zeroing raced with other CTAs' in-flight adds / reads on
        # PPU-ZW810E (gpu-scope atomic arrival order is not reliable across
        # the volatile-spin barrier): round>=3 reads could see stale data
        # from round 0, corrupting the pivot (observed low byte 0xC0 = round
        # 0's bin). Buffers are zeroed once per kernel launch by the host.
        current_hist_ptr = g_histogram_ptr + round_idx * RADIX
        ...
        for t in tl.range(0, n_vec_full):
            ...
            mask = _radix_prefix_mask(round_idx)
        ...
        barrier_phase += 1
        # PPU fix (2026-08-31): the first barrier's volatile-spin release
        # does not guarantee that other CTAs' relaxed gpu-scope histogram
        # adds are globally visible when this CTA reads the histogram below
        # (PPU-ZW810E: relaxed atomics can still be in flight after the
        # spin; observed per-CTA pivot split -> unmasked gt-store overflow
        # into the next row's output). A second barrier widens the window
        # for in-flight adds to land before the read. Verified: r4 k2048
        # 1000+ launches 0 failures (was 30-37.5%), 10/10 test rounds pass.
        _barrier_with_atomic_add(
            g_state_ptr + 2,
            zeros,
            lane,
    ...
    for t in tl.range(0, n_vec_full):
        ...
        tl.store(
            row_output + eq_pos,
            my_chunk_start + offs,
            mask=eq_mask & (eq_pos < tl.full((), TOPK, tl.uint32)),
        )
    for t in tl.range(0, rem_tiles):
        offs = (n_vec_full * VEC_SIZE + t) * BLOCK_SIZE + lane
        ordered = tl.load(shared_ordered_ptr + offs)
...
def persistent_topk_kernel(
    ...
    g_histogram_base = g_histogram_ptr
    g_state_base = g_state_ptr
    ...
    for i in tl.range(total_iters):
        ...
        if row_idx < num_rows:
            ...
            # PPU: each row gets its own 4-buffer histogram group and its own
            # barrier/output state (zeroed once by the host per launch).
            g_histogram_ptr = g_histogram_base + row_idx * 4 * RADIX
            g_state_ptr = g_state_base + row_idx * 4
            barrier_phase = tl.zeros((), dtype=tl.uint32)
...
def persistent_topk(
    ...
    # PPU routing: persistent kernel (multi-CTA cooperative radix + decode/
    # medium single-CTA) for num_rows <= 32, per-row single-CTA kernel
    # (per_row_cta path) for num_rows > 32 — matching the generic
    # implementation's naming and dispatch.
    # Correctness: the cooperative radix path carries the double-barrier
    # fix (see _radix_topk) for the PPU-ZW810E relaxed gpu-atomic
    # visibility limitation; see 2026_08_25_persistent_topk_ppu_v0_2_log.md
    # for the analysis and platform limitation notes.
    if num_rows > 32:
    ...
    # PPU smem-tier experiment (2026-09-02): the generic rows<=4 cap of
    # SMEM_MEDIUM (35KB -> cpg=32) is suboptimal on PPU-ZW810E — a 32-CTA
    # group's barrier wait (double barrier needs all 32 to arrive) costs
    # more than the extra threads save. Measured (seq=262144, k=512/2048,
    # do_bench): rows=1 cpg16 1.61x vs cpg32 1.35x; rows>=2 cpg8 1.38-1.56x
    # vs cpg32 0.75-1.18x. So: rows==1 uses the 96KB tier (cpg=16, best for
    # a single row), rows>=2 uses the full 256KB tier (cpg=8, more groups ->
    # rows 2-4 no longer serialize into 2 waves). SMEM_MEDIUM (medium-path
    # layout requirement) is retained for reference; smem_size below is
    # unused by the Triton launch (tle.gpu.alloc derives smem at compile
    # time), kept to mirror the generic host math.
    if num_rows == 1:
        effective_max_smem = min(max_smem_per_block, SMEM_TIER_R1)
    else:
    ...
    if needs_cooperative:
        # PPU tuning (Trial 3): drop the generic headroom reservation — the
        # cooperative launch is the only kernel on the stream and all 64 CTAs
        # fit on 64 SMs (verified: no deadlock, correctness 0 missed). One
        # extra group (e.g. rows=8 -> 8 groups x 8 CTAs) removes per-group
        # row serialization; rows=8/16/32 speedup 0.70->0.87 / 0.80 / 0.76.
        headroom = 0 if occupancy == 1 else 1
    ...
    #     int arrival_counter;
    #     int output_counter;
    histogram_bytes = RADIX * 4 * 4
    hist_total = num_rows * histogram_bytes
    g_state_size = num_rows * 4 * 4
    assert workspace.size(0) >= hist_total + g_state_size
    workspace[: (hist_total + g_state_size)] = 0
    g_histogram_size = hist_total
    g_histogram = (
        workspace[:g_histogram_size].view(torch.uint32).view(num_rows, 4, RADIX)
    )
    g_state = (

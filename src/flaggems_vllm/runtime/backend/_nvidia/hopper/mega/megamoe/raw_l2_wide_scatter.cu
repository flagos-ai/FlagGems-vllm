...
// dst_rows_smem by Triton, so the CUDA boundary does not need any global
// pointer argument at all.
extern "C" __device__ void TleL2WideScatter(const uint64_t values_smem,
                                            const uint64_t dst_rows_smem,
                                            const int valid_rows,
                                            const int cols, const int bar_id) {
...
#pragma unroll 1
  for (int j = 0; j < 8; ++j) {
    ...
    if (row < valid_rows) {
      ...
      asm volatile("cvta.to.shared.u64 %0, %7;\n"

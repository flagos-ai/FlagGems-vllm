...
def interleave_l1_gate_up_rows(weight, torch, granularity: int = 8):
    ...
    if half % granularity:
        raise ValueError(
            f"L1 rows per gate/up half must be divisible by {granularity}, got {half}"
        )
...
def load_qwen3_rank_data(
    ...
    *,
    ...
    if type(dataset_tokens) is not int or dataset_tokens <= 0:
        raise ValueError(
            f"manifest tokens_per_rank must be a positive integer, got {dataset_tokens!r}"
        )
    if num_tokens <= 0 or num_tokens > dataset_tokens:
        raise ValueError(
            "shared-data token capacity is insufficient: "
    ...
    if num_experts % num_ranks:
        raise ValueError(
            f"num_experts={num_experts} is not divisible by num_ranks={num_ranks}"
        )
    ...
    for name, (collection, dtype, tensor_shape) in expected_tensors.items():
        ...
        if tensor.dtype != dtype or tuple(tensor.shape) != tensor_shape:
            raise ValueError(
                f"{name}: got dtype={tensor.dtype}, shape={tuple(tensor.shape)}; "
    ...
    if not torch.allclose(
        ...
        raise ValueError(
            "Qwen3 top-k routing weights are not normalized within the "

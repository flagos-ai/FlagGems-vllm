...
def _zero_output_kernel(
    ...
    row = offsets // N % M
    col = offsets % N
    tl.store(Z + batch * SZB + row * SZM + col * SZN, 0, offsets < NUMEL)


def _zero_output(z):
    # Fixed-size initialization is not a compute tuning path.
    _zero_output_kernel[(triton.cdiv(z.numel(), 1024),)](
        z, z.shape[1], z.shape[2], *z.stride(), z.numel(), BLOCK=1024
    )
...
def w8a8_block_fp8_bmm(
    ...
    use_tle = (
        HAS_TLE_W8A8_BLOCK_FP8_BMM
        and y.is_contiguous()
        and ys.is_contiguous()
        and x.data_ptr() % 16 == 0
        and y.data_ptr() % 16 == 0

...
def _glm5_next_kpool_state_compress_kernel(
    ...
    for i in tl.static_range(T_PER_PROG):
        ...
        if token_idx < num_tokens:
            ...
            k_row = tl.load(
                k_ptr + token_idx * k_stride_t + dim_offsets, mask=dim_mask, other=0.0
            )
            gate_row = tl.load(
            ...
            prev_query_end = tl.load(
                cum_query_lens_ptr + req_id - 1, mask=req_id > 0, other=0
            )
...
def glm5_next_kpool_state_compress_and_write_cache_triton(
    ...
    logger.debug("GEMS_ASCEND KPOOL_STATE_COMPRESS")

...
# Reusable intermediate buffers (pooled per dtype/device, numel only grows).
# Ported from the nopad path (2026-08-13 engineering optimisation): the
# generic implementation allocates cache13 + cache2 every call (~804MB for
# M=16384/I=512), which dominates host time on large-M shapes. Buffers are
# fully written (or only valid rows read), so reuse needs no clearing.
_CACHE13: dict = {}
_CACHE2: dict = {}
...
def _get_cached_buffer(cache: dict, min_numel: int, device, dtype) -> torch.Tensor:
    ...
    if buf is None or buf.numel() < min_numel:
        buf = torch.empty(min_numel, device=device, dtype=dtype)
        cache[key] = buf
    return buf
...
def _can_use_nopad_fp8_moe(
    ...
    """Route large-M blockwise-fp8 MoE to the nopad kernel.

    Triton implementation of the MUSA DeepGemm scheme (permute + grouped fp8
    GEMM with contiguous rows and per-step fp32 scale fold): fp8 dot is the
    MUSA fast path (faster than bf16), and the padded kernel's per-step
    scattered a_scale gather is avoided by the nopad row-contiguous layout.
    """
    return (
        use_fp8_w8a8
        and block_shape is not None
        and hidden_states.dtype == torch.bfloat16
        and hidden_states.size(0) >= 8192
        and w1.size(1) <= 1024  # 2 * I, I <= 512 (已验证范围)
        and activation == "silu"
...
def _can_use_nopad_moe(
    ...
    """Route bf16 large-M Qwen3.6-35B (I=128) MoE to the nopad kernel.

    MTT S5000 tuning (2026-08-13/14): M>=8192 + half + I<=512 + silu is the
    production real-world large-M shape; nopad (independent scheduler, no
    sentinel rows) is ~2x faster than the padded path and stable across
    runs. Falls back to the padded kernel for everything else.
    """
    return (
        hidden_states.dtype in (torch.bfloat16, torch.float16)
        and hidden_states.size(0) >= 8192
        and w1.size(1) <= 1024  # 2 * I, I <= 512 (Qwen3.6 生产系——已验证范围)
        and activation == "silu"
        and not apply_router_weight_on_input
        ...
        for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
            ...
            if use_fp8_w8a8 or use_int8_w8a8:
                if group_k > 0 and group_n > 0:
                    ...
                    if BLOCK_SIZE_N <= group_n:
                        combined = a_scale[:, None] * b_scale_val
                    else:
                        combined = a_scale[:, None] * b_scale_val[None, :]
                    acc_gate += tl.dot(a, b_gate) * combined
                else:
                    if use_fp8_w8a8:
            ...
            if use_int8_w8a16:
                ...
                if group_k > 0 and group_n > 0:
                    ...
                    if SWAP_AB:
                        ...
                        if BLOCK_SIZE_N <= group_n:
                            combined = b_scale_val * a_scale[None, :]
                        else:
                            combined = b_scale_val[:, None] * a_scale[None, :]
                        accumulator_nm += tl.dot(tl.trans(b), tl.trans(a)) * combined
                    else:
                        b_scale_val = tl.load(b_scale_ptrs + offs_ks * stride_bsk)
                        if BLOCK_SIZE_N <= group_n:
                            combined = a_scale[:, None] * b_scale_val
                            ...
                            combined = a_scale[:, None] * b_scale_val[None, :]
                        accumulator += tl.dot(a, b) * combined
                else:
                    if use_fp8_w8a8:
                        if SWAP_AB:
...
def fused_experts_impl(
    ...
    if _can_use_nopad_moe(
        ...
        return nopad_fused_moe(
            hidden_states, w1, w2, topk_weights, topk_ids, inplace=inplace
        )
    ...
    intermediate_cache2 = _get_cached_buffer(
        _CACHE2,
        M * top_k_num * activation_out_dim,
        hidden_states.device,
        hidden_states.dtype,
    )[: M * top_k_num * activation_out_dim].view(M * top_k_num, activation_out_dim)
...
# =============================================================================
# Nopad fused MoE (integrated from nopad_moe.py -- 2026-08-14)
# Independent scheduler (no sentinel rows) for large-M half MoE. See
# _can_use_nopad_moe above for the routing condition.
# =============================================================================
"""
Pure-Triton nopad grouped-GEMM fused MoE pipeline (no external GEMM library).

Implements the DeepGemm GroupedNoPad scheduling idea in Triton: each program
handles a contiguous row block of a single expert, with rows indexed through
``sorted_ids`` and the expert weight base = ``w1 + expert * N * K``. No global
row padding is introduced (padding only within each expert's tail block).

MUSA engineering optimizations (vs the PPU baseline port):
- Buffer reuse: meta/data cached by (M, device) key -- no per-call ~300MB alloc
- Minimal zeroing: only counts/counters cleared per call (~1KB)
- counts.max().item() called once (shared by gemm1/gemm2 grids)
- GEMM1 gate+up fused via FUSE_SILU (single launch incl. activation; no
  separate silu kernel, no second up pass, no column-offset path)
  -> 6 launches total

Pipeline (all Triton):
    permute (histogram -> cumsum -> assign) -> nopad gemm1 (+FUSE_SILU)
      -> nopad gemm2 -> unpermute + top-k weighted reduce
"""
...
@triton.jit
def _histogram_kernel(topk_ids_ptr, counts_ptr, numel, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
...
def _exclusive_cumsum_kernel(
    counts_ptr, start_ptr, num_experts: tl.constexpr, BLOCK: tl.constexpr
):
    offs = tl.arange(0, BLOCK)
    mask = offs < num_experts
    c = tl.load(counts_ptr + offs, mask=mask, other=0).to(tl.int32)
    s = tl.cumsum(c, axis=0) - c
    tl.store(start_ptr + offs, s, mask=mask)
    tl.store(start_ptr + num_experts, tl.sum(c, axis=0))
...
def _assign_kernel(
    ...
    mask = offs < numel
    e = tl.load(topk_ids_ptr + offs, mask=mask, other=0)
    start = tl.load(start_ptr + e, mask=mask, other=0)
    local = tl.atomic_add(counters_ptr + e, 1, mask=mask)
    pos = start + local
    t = offs // topk
    j = offs % topk
    tl.store(sorted_ids_ptr + pos, t, mask=mask)
    tl.store(inv_perm_ptr + t * topk + j, pos, mask=mask)
...
def _unpermute_kernel(
    ...
    offs_h = pid_h * BLOCK + tl.arange(0, BLOCK)
    mask_h = offs_h < hidden
    acc = tl.zeros((BLOCK,), dtype=tl.float32)
    for j in tl.static_range(topk):
        row = tl.load(inv_perm_ptr + pid_t * topk + j)
        w = tl.load(weights_ptr + pid_t * topk + j)
        vals = tl.load(src_ptr + row * hidden + offs_h, mask=mask_h, other=0)
        acc += vals.to(tl.float32) * w.to(tl.float32)
    tl.store(
        out_ptr + pid_t * hidden + offs_h,
...
def _nopad_fused_moe_kernel(
    ...
    """
    Nopad grouped fused MoE kernel (gemm1 with fused SiLU, or gemm2).
    Rows are contiguous per expert; each program handles one
    (expert, n_tile, m_tile) block. With GATHER, a-tiles are gathered from
    the pre-permute activations through ``sorted_ids`` inside the kernel;
    otherwise a is expert-contiguous.
    """
    n_out = N // 2 if FUSE_SILU else N
    num_n_tiles = tl.cdiv(n_out, BLOCK_N)
    pid = tl.program_id(0)
    expert = pid // (num_n_tiles * NUM_M_TILES)
    rem = pid % (num_n_tiles * NUM_M_TILES)
    n_tile = rem // NUM_M_TILES
    m_tile = rem % NUM_M_TILES

    cnt = tl.load(counts_ptr + expert)
    row_base = tl.load(start_ptr + expert)
    m_start = row_base + m_tile * BLOCK_M
    if m_start >= row_base + cnt:
        return

    offs_m = tl.arange(0, BLOCK_M)
    offs_n = n_tile * BLOCK_N + tl.arange(0, BLOCK_N)
    offs_k = tl.arange(0, BLOCK_K)
    valid = (m_start + offs_m) < (row_base + cnt)
    if GATHER:
        token_ids = tl.load(sorted_ids_ptr + m_start + offs_m, mask=valid, other=0)
    ...
    b_mask = offs_n[None, :] < n_out
    acc_gate = tl.zeros((BLOCK_M, BLOCK_N), dtype=tl.float32)
    acc_up = tl.zeros((BLOCK_M, BLOCK_N), dtype=tl.float32)
    if FP8_QUANT:
        # Blockwise fp8: A and B are fp8; the per-group scales are applied
        # per K-step in fp32 (fp8 dot with fused acc cannot fold the scales).
        # With GATHER the a_scale is indexed by token (scattered within the
        # expert's small row window); otherwise it is row-contiguous.
        if GATHER:
            scale_rows = token_ids
        else:
            scale_rows = m_start + offs_m
        b_scale_base = (
            b_scale_ptr + expert * stride_bse + offs_n[None, :] // 128 * stride_bsn
        )
        if FUSE_SILU:
            b_scale_up_base = (
                b_scale_ptr
                + expert * stride_bse
                + (offs_n[None, :] + n_out) // 128 * stride_bsn
            )
        a_scale_base = a_scale_ptr + scale_rows[:, None] * stride_asm
        for k in range(0, tl.cdiv(K, BLOCK_K)):
            a = tl.load(a_ptrs, mask=valid[:, None], other=0.0)
            b_gate = tl.load(b_ptrs, mask=b_mask, other=0.0)
            offs_ks = (k * BLOCK_K) // 128
            a_scale = tl.load(
                a_scale_base + offs_ks * stride_ask, mask=valid[:, None], other=0.0
            )
            b_scale = tl.load(b_scale_base + offs_ks * stride_bsk)
            combined = a_scale * b_scale
            acc_gate += tl.dot(a, b_gate) * combined
            if FUSE_SILU:
                b_up = tl.load(b_ptrs_up, mask=b_mask, other=0.0)
                b_scale_up = tl.load(b_scale_up_base + offs_ks * stride_bsk)
                acc_up += tl.dot(a, b_up) * (a_scale * b_scale_up)
            a_ptrs += BLOCK_K * stride_ak
            b_ptrs += BLOCK_K * stride_bk
            b_ptrs_up += BLOCK_K * stride_bk
        if FUSE_SILU:
            acc_gate = acc_gate * tl.sigmoid(acc_gate) * acc_up
    else:
        for k in range(0, tl.cdiv(K, BLOCK_K)):
            a = tl.load(a_ptrs, mask=valid[:, None], other=0.0)
            b_gate = tl.load(b_ptrs, mask=b_mask, other=0.0)
    ...
    c_ptrs = (
        c_ptr + (m_start + offs_m)[:, None] * stride_cm + offs_n[None, :] * stride_cn
    )
    tl.store(
        c_ptrs,
        acc_gate.to(c_ptr.dtype.element_ty),
        mask=valid[:, None] & (offs_n[None, :] < n_out),
    )
...
def _transposed(w, cache):
    """Cache the k-major transpose (E, K, N) of a (E, N, K) weight."""
    key = (w.data_ptr(), w.shape, w.dtype)
    entry = cache.get(key)
    if entry is not None and entry[0]() is w:
        return entry[1]
    t = w.permute(0, 2, 1).contiguous()
    cache[key] = (__import__("weakref").ref(w), t)
    return t


def _get_buffers(M, E, H, n_inter, topk, device, dtype):
    """Reusable meta/data buffers (keyed by shape); only counts/counters cleared."""
    numel = M * topk
    key = (M, E, H, n_inter, topk, device, dtype)
    if key not in _META_CACHE:
        meta = torch.zeros(
            3 * E + 1 + 2 * numel + M * topk, dtype=torch.int32, device=device
        )
        _META_CACHE[key] = meta
        data = torch.empty(2 * numel * n_inter + numel * H, dtype=dtype, device=device)
    ...
    counts = meta[:E]
    start = meta[E : 2 * E + 1]
    counters = meta[2 * E + 1 : 3 * E + 1]
    sorted_ids = meta[3 * E + 1 : 3 * E + 1 + numel]
    inv_perm = meta[3 * E + 1 + 2 * numel :].view(M, topk)
    gate = data[: numel * n_inter].view(numel, n_inter)
    cache3 = data[numel * n_inter : numel * n_inter + numel * H].view(numel, H)
    return counts, start, counters, sorted_ids, inv_perm, gate, cache3
...
def nopad_fused_moe(
    ...
    """Pure-Triton nopad fused MoE (bf16/fp16 or blockwise fp8, silu, no bias/EP)."""
    dtype = hidden_states.dtype
    M, H = hidden_states.shape
    E, N, K = w1.shape  # N = 2 * intermediate
    n_inter = N // 2
    ...
    if cfg_g1 is None:
        cfg_g1 = dict(_NOPAD_CFG)
    if cfg_g2 is None:
        cfg_g2 = dict(_NOPAD_CFG)

    counts, start, counters, sorted_ids, inv_perm, gate, cache3 = _get_buffers(
        M, E, H, n_inter, topk, device, dtype
    )
    out = hidden_states if inplace else torch.empty((M, H), device=device, dtype=dtype)

    ti_flat = topk_ids.reshape(-1)
    _histogram_kernel[(triton.cdiv(numel, _HIST_BLOCK),)](
        ti_flat, counts, numel, BLOCK=_HIST_BLOCK
    )
    _exclusive_cumsum_kernel[(1,)](counts, start, E, BLOCK=triton.next_power_of_2(E))
    _assign_kernel[(triton.cdiv(numel, _ASSIGN_BLOCK),)](
        ti_flat,
        start,
        counters,
        sorted_ids,
    ...
    cnt_max = int(counts.max().item())

    fp8_quant = use_fp8_w8a8 and block_shape is not None
    if fp8_quant:
        from flaggems_vllm.ops.per_token_group_quant_fp8 import (
            per_token_group_quant_fp8,
        )

        # MUSA capability is (3,1), so the generic fp8-support probe picks
        # float32; force fp8 (MUSA fp8 conversion works) for the fp8 dot path.
        a_quant, a1_scale = per_token_group_quant_fp8(
            hidden_states, group_size=block_shape[1], dtype=torch.float8_e4m3fn
        )
    else:
        a_quant = hidden_states
        a1_scale = None

    w1T = _transposed(w1, _W1T_CACHE)
    w2T = _transposed(w2, _W2T_CACHE)

    # GEMM1: single launch with FUSE_SILU (gate+up inside, SiLU in epilogue,
    # output = activated intermediate written to `gate`). Kernel N argument is
    # the full 2*I so n_out = N//2 = I inside.
    nmt = triton.cdiv(cnt_max, cfg_g1["BLOCK_M"])
    grid1 = (E * triton.cdiv(n_inter, cfg_g1["BLOCK_N"]) * nmt,)
    _nopad_fused_moe_kernel[grid1](
        a_quant,
        w1T,
        gate,
        start,
        ...
        None,
        sorted_ids,
        a1_scale if fp8_quant else None,
        w1_scale if fp8_quant else None,
        a1_scale.stride(0) if fp8_quant else 0,
        a1_scale.stride(1) if fp8_quant else 0,
        w1_scale.stride(0) if fp8_quant else 0,
        w1_scale.stride(1) if fp8_quant else 0,
        w1_scale.stride(2) if fp8_quant else 0,
        numel,
        2 * n_inter,
        H,
        E,
        ...
        FUSE_SILU=True,
        FP8_QUANT=fp8_quant,
        BLOCK_M=cfg_g1["BLOCK_M"],
        BLOCK_N=cfg_g1["BLOCK_N"],
        BLOCK_K=cfg_g1["BLOCK_K"],
        GROUP_M=cfg_g1["GROUP_M"],
        num_warps=cfg_g1["num_warps"],
        num_stages=cfg_g1["num_stages"],
    )

    # GEMM2: contiguous a (activated gate), single pass
    if fp8_quant:
        from flaggems_vllm.ops.per_token_group_quant_fp8 import (
            per_token_group_quant_fp8,
        )

        a2_quant, a2_scale = per_token_group_quant_fp8(
            gate, group_size=block_shape[1], dtype=torch.float8_e4m3fn
        )
    else:
        a2_quant = gate
        a2_scale = None
    nmt2 = triton.cdiv(cnt_max, cfg_g2["BLOCK_M"])
    grid2 = (E * triton.cdiv(H, cfg_g2["BLOCK_N"]) * nmt2,)
    _nopad_fused_moe_kernel[grid2](
        a2_quant,
        w2T,
        cache3,
        start,
        ...
        None,
        None,
        a2_scale if fp8_quant else None,
        w2_scale if fp8_quant else None,
        a2_scale.stride(0) if fp8_quant else 0,
        a2_scale.stride(1) if fp8_quant else 0,
        w2_scale.stride(0) if fp8_quant else 0,
        w2_scale.stride(1) if fp8_quant else 0,
        w2_scale.stride(2) if fp8_quant else 0,
        numel,
        H,
        n_inter,
        E,
        ...
        FUSE_SILU=False,
        FP8_QUANT=fp8_quant,
        BLOCK_M=cfg_g2["BLOCK_M"],
        BLOCK_N=cfg_g2["BLOCK_N"],
        BLOCK_K=cfg_g2["BLOCK_K"],
        GROUP_M=cfg_g2["GROUP_M"],
        num_warps=cfg_g2["num_warps"],
        num_stages=cfg_g2["num_stages"],
    )

    _unpermute_kernel[(M, triton.cdiv(H, _REDUCE_BLOCK))](
        cache3, out, inv_perm, topk_weights, M, H, topk=topk, BLOCK=_REDUCE_BLOCK
    )
    return out

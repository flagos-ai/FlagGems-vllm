...
"""Mthreads S-series-optimized Qwen4 compress+norm+mrope+store fused kernel.
...
"""Qwen4 fused QSA group-pool / Gemma-RMSNorm / MRoPE / store kernel.

Self-contained Triton module. Entry point: run(...).
"""
...
def _compress_norm_mrope_store_qsa_groups_kernel(
    ...
    for group_offset in tl.range(0, COMPRESS_RATIO):
        ...
        safe_logical_page = tl.minimum(tl.maximum(logical_page, 0), RAW_TABLE_WIDTH - 1)
        safe_page_offset = tl.minimum(tl.maximum(page_offset, 0), RAW_PAGE_SIZE - 1)
        valid = valid_row & (logical_page >= 0) & (logical_page < RAW_TABLE_WIDTH)
        ...
        accumulator += tl.load(
            ...
            + safe_page_offset * stride_raw_token
            + dims * stride_raw_dim,
    ...
    if LOAD_MROPE_POSITIONS:
        ...
        time_position = tl.load(rope_base, mask=valid_rope, other=0)
        height_position = tl.load(rope_base + stride_rope_dim, mask=valid_rope, other=0)
        width_position = tl.load(rope_base + 2 * stride_rope_dim, mask=valid_rope, other=0)
    else:

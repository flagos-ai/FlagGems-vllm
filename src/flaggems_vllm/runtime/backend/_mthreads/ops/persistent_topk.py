...
# MTT S5000 max threads per block; MUSA device properties do not expose it.
MAX_THREADS_PER_BLOCK = 1024
# v0.2: smem budget tiers per num_rows bucket (bytes). v0.2 sweep on MTT S5000
# (rows 1-8 x 48/96/160/192KB, stride=262144) showed the generic 36KB/48KB caps
# force cpg=32/chunk=8192/ng=1 (single sequential group, SpeedUp 0.59-0.78 on
# rows 5-8), while the full budget gives cpg=8/chunk=32768/multi-group
# parallelism (SpeedUp 1.59-2.83). Both tiers relaxed to the full budget;
# decode/medium (non-cooperative) paths verified regression-free.
SMEM_TIER_LE4 = 196608
SMEM_TIER_LE8 = 196608
...
def persistent_topk(
    ...
    """
    assert logits.device.type == "musa", "persistent_topk: logits must be MUSA tensor"
    assert lengths.device.type == "musa", "persistent_topk: lengths must be MUSA tensor"
    assert output.device.type == "musa", "persistent_topk: output must be MUSA tensor"
    ...
    # MUSA `_MusaDeviceProperties` only exposes a few fields (multi_processor_count
    # etc.); shared-memory and thread limits come from the Triton MUSA driver.
    device_props = torch_device_fn.get_device_properties(device.index)
    num_sms = device_props.multi_processor_count
    triton_props = triton.runtime.driver.active.utils.get_device_properties(
        device.index
    )
    max_smem_per_block = triton_props["max_shared_mem"]
    # v0.2: shared-memory budget tiers by num_rows bucket. The generic 36KB/48KB
    # caps for small batches force cpg=32 / chunk=8192 / ng=1 (single sequential
    # group) on MTT S5000; the full budget gives cpg=8 / chunk=32768 with
    # multi-group parallelism (SpeedUp 0.59-0.78 -> 1.68-2.83 on rows 5-8).
    if num_rows <= 4:
        effective_max_smem = min(max_smem_per_block, SMEM_TIER_LE4)
    elif num_rows <= 8:
        effective_max_smem = min(max_smem_per_block, SMEM_TIER_LE8)
    else:

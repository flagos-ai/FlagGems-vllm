...
"""TopK Softplus-Sqrt gating kernel in Triton (T-Head backend).

Design notes
------------
* One program handles one token (one row of ``gating_output``) with
  ``num_warps=1``, mirroring the CUDA kernel's one-warp-per-row layout.
* The top-k loop repeatedly extracts the row maximum. Ties are broken towards
  the smallest expert index, which matches ``torch.topk`` and is deterministic.
* The un-biased weight is recovered as ``max_score - bias[eidx]`` so only one
  score tensor has to be kept live.
...
They are algorithmically identical to the baseline kernels; the only difference
is that intermediate state is staged in explicit shared memory (via
``tle.gpu.alloc`` / ``tle.gpu.local_ptr``) instead of round-tripping through
global memory:

  - Dense path (``_fused_topk_kernel_tle``): the store -> load -> store pattern
    used by the baseline for the two-pass renormalization is replaced by a
    shared-memory scratch buffer, so only one global store of the weights
    happens at the end.
  - Hash path (``_hash_kernel_tle``): the O(BLOCK_E) masked-reduction gather
    ...
    post-transform scores in shared memory once and doing O(1) indexed loads
    per top-k slot.

``topk_softplus_sqrt`` dispatches to the TLE kernels automatically when they
are available. ``topk_softplus_sqrt_baseline`` / ``topk_softplus_sqrt_tle``
force a specific variant (for testing and benchmarking).
"""
...
def _fused_topk_kernel(
    ...
    for k_idx in tl.static_range(topk):
        ...
        # Remove the winner from the candidate set
...
# ---------------------------------------------------------------------------
# TLE variants (Triton experimental TLE API)
#
...
# through global memory is staged in explicit shared memory via
...
if HAS_TLE:
    ...
    def _fused_topk_kernel_tle(
        ...
        # Shared-memory scratch for the top-k weights. Replaces the global
        # store -> (later) load -> store used for renormalization in the
        # baseline kernel.
        ...
        for k_idx in tl.static_range(topk):
            ...
            weight_sum += w
            # Single-element store into shared memory at slot k_idx.
            tl.store(
        ...
        # One read-back from shared memory + one global store of final weights.
        kmask = k_range < topk
    ...
    def _hash_kernel_tle(
        ...
        # Stage the post-transform scores in shared memory once, so each of
        # the `topk` gathers below is an O(1) indexed load instead of an
        # O(BLOCK_E) masked reduction over the whole row.
...
def _topk_tle_available(gating_output) -> bool:
    """Whether the TLE shared-memory kernels can be used for this input.

    Requires the TLE API to be importable and a CUDA/MUSA-style device tensor.
    """
    return HAS_TLE and gating_output.device.type in ("cuda", "musa")
...
def topk_softplus_sqrt(
    ...
    When Triton's TLE API is available and the input is on a supported device,
    the shared-memory-staged TLE variant is used automatically; the output is
    equivalent to the baseline kernel, only the memory access pattern differs.
    Use ``topk_softplus_sqrt_baseline`` / ``topk_softplus_sqrt_tle`` to force a
    specific variant (e.g. for benchmarking/testing).
    """
...
def topk_softplus_sqrt_tle(
    ...
    kernel. Raises if TLE is unavailable (no silent fallback), so
    benchmarks/tests fail loudly instead of quietly measuring the baseline.
    """
    logger.debug("GEMS TOPK_SOFTPLUS_SQRT (forced TLE)")
    if not _topk_tle_available(gating_output):
        raise RuntimeError(
            "topk_softplus_sqrt_tle requires Triton TLE support "
            "(triton.experimental.tle) and a CUDA/MUSA tensor; "
